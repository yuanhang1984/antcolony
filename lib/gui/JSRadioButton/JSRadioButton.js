...
class JSRadioButton {
  ...
  generateCode() {
    ...
    this.objectCode = `
      <input ${selectedCode} ${enabledCode} type = "radio" class = "${this.getClass()}" id = "${this.getId()}" name = "${this.getName()}" value = "${this.getValue()}" /><label for = "${this.getId()}">${this.getLabel()}</label>
    `;
  }
}
