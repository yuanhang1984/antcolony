...
class JSCheckBox {
  ...
  generateCode() {
    ...
    this.objectCode = `
      <input ${selectedCode} ${enabledCode} type = "checkbox" class = "${this.getClass()}" id = "${this.getId()}" name = "${this.getName()}" value = "${this.getValue()}" /><label for = "${this.getId()}">${this.getLabel()}</label>
    `;
  }
}
