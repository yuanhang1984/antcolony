...
class JSRadioButton {
  ...
  update() {
    ...
    $(`#${this.getId()}`).change(function() {
      if($(this).is(":checked")) {
        _this.objectSelected = true;
      } else {
        _this.objectSelected = false;
      }
    });
  }
  ...
  generateCode() {
    ...
    this.objectCode = `
      <input ${selectedCode} ${enabledCode} type = "radio" class = "JSRadioButton ${this.getClass()}" id = "${this.getId()}" name = "${this.getName()}" value = "${this.getValue()}" /><label class = "JSRadioButton ${this.getClass()}" for = "${this.getId()}">${this.getLabel()}</label>
    `;
  }
}
